...
adj_tensor[, , 2] # Print the first sample of the posterior distribution over adjacency matrices
```
...
for (i in 1:num_iterations) {
  g <- graph_from_adjacency_matrix(adj_tensor_transformed[, , i], mode="undirected", weighted=TRUE)
...
loglik <- function(params, Y, X, index) {
  ...
  target <- target + dnorm(beta_lifeform, mean=0, sd=0.5, log=TRUE) # Prior on lifeform coefficient
  target <- target + dnorm(beta_droid, mean=0, sd=0.5, log=TRUE) # Prior on droid coefficient
...
loglik_dyadreg <- function(params, Y, X, index) {
  ...
  # Define model
  target <- 0
  target <- target + sum(dnorm(y, mean=intercept + beta_dyadtype[X[, 1]] + (mm[X[, 2]] + mm[X[, 3]]) * mm_sigma, sd=sigma, log=TRUE)) # Main model
  target <- target + dnorm(intercept, mean=0, sd=0.5, log=TRUE) # Prior on intercept
  target <- target + sum(dnorm(beta_dyadtype, mean=0, sd=0.5, log=TRUE)) # Prior on dyadtypes coefficient
  target <- target + dexp(sigma, rate=1, log=TRUE) # Prior on sigma
  
  # Node multimembership model
  target <- target + sum(dnorm(mm, mean=0, sd=1, log=TRUE))
  target <- target + dexp(mm_sigma, rate=1, log=TRUE)
...
```{r}
chain <- metropolis(target, rep(0, 35), iterations=100000, thin=100, refresh=10000)
# colnames(chain) <- c("intercept", "beta_lifeform", "beta_droid", "sigma")
head(chain)
```
